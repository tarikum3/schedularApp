...
import {
 endOfYear,startOfYear,subYears
} from "date-fns";
...
const DateWrapper: React.FC<CustomTableProps> = ({ children ,onTableDateRangeChange,title,type}) => {
  ...
  useEffect(() => {
      ...
      if (type!="year"&&dateRange?.startDate && dateRange?.endDate) {
        ...
        setToDatePrev(dateFormatDays(subYears(new Date(dateRange?.endDate),1), "YYYY-MM-DD"))
          ...
          setToDate("")
        setFromDate("") 
        setFromDatePrev(dateFormatDays( startOfYear(subYears(new Date(),1)), "YYYY-MM-DD"))
        setToDatePrev(dateFormatDays( endOfYear(subYears(new Date(),1)), "YYYY-MM-DD"))
        }
    ...
    useEffect(() => {
        ...
        if (type &&type=="year"&&selectedYear) {
          ...
          setFromDatePrev(dateFormatDays( startOfYear(new Date(selectedYear-1, 0, 1)), "YYYY-MM-DD"))
          setToDatePrev(dateFormatDays( endOfYear(new Date(selectedYear-1, 0, 1)), "YYYY-MM-DD"))
    ...
    useEffect(() => {
        ...
        if(onTableDateRangeChange){
          onTableDateRangeChange({startDate:fromDate,endDate:toDate,
            startDatePrev:fromDatePrev,endDatePrev:toDatePrev,});
        }
...
/>
</LocalizationProvider></div>:<DateRangeComponent
              open={dateRangeModal}
