...
const CustomDateRangeComponent = ({ open, onChange, toggle, label}: Props) => {
  ...
  return (
    <div className="">
      ...
      <div className="relative flex items-end  bg-white text-[#2C2E7B]  w-full max-w-[200px] ml-auto rounded-[3px] text-md border border-gray-300 ">
        <div onClick={() => toggle(true)}>
          ...
          value={`${
              ...
              dateRange?.endDate ? dateFormatDays(dateRange.endDate) : label ? "" :"To"
            }`}
          />
        </div>
      {(dateRange?.startDate ||dateRange?.endDate) &&(<div className="w-full h-full flex-1 py-[7px] pr-2 bg-">
